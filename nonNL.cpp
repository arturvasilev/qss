...
// nlopt::func convert ( myfunc f, graph &g)
// {
// 	nlopt::func tmp =  [&g, f](const vector<double> &x, vector<double> &grad, void *data) -> double
// 	{
// 		g.var = x;
// 		return abs(f());
// 	};
...
double NLopt (graph &g, myfunc obj_myfunc, vector<myfunc> &restrictions, double eps)
{
	...
	tmp.front().g = &g;
	tmp.front().f = &obj_myfunc;
	for(size_t i = 1; i < restrictions.size(); i++)
	{
		tmp[i].g = &g;
		tmp[i].f = &restrictions[i];
	}
    glob_problem.set_max_objective( 
		[](const vector<double> &x, vector<double> &grad, void *data) -> double
		{
			dummy *loc  = reinterpret_cast<dummy*>(data);
			loc -> g -> var = x;
			return abs((*loc -> f)());
		}, &tmp.front());
    //Устанавливаем границы изменения переменных
    vector<double> lb(dim, -1), ub(dim, 1);
    glob_problem.set_lower_bounds(lb);
    glob_problem.set_upper_bounds(ub);
	for (auto rest : restrictions)
		glob_problem.add_equality_constraint(
			[](const vector<double> &x, vector<double> &grad, void *data) -> double
		{
			dummy *loc  = reinterpret_cast<dummy*>(data);
			loc -> g -> var = x;
